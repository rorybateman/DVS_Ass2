...
main_image = cv2.imread('speed_photos/Beeswing.png')

template = cv2.imread('Template_speeds/40.jpeg')

# Methods
# 1. Template Matching
    # Does not work well with rotation and scaling
...
# Tune the threshold to get 2 matches
# Start with a threshold value
threshold = 0.8
matches = 0
while matches != 2:
    result_path, matches = template_matching('/mnt/data/Beeswing.png', '/mnt/data/40.jpeg', threshold)
    # Adjust the threshold based on the number of matches
    if matches > 2:
        threshold += 0.05
    elif matches < 2:
        threshold -= 0.05
...
# extract_img = preprocess(main_image) # Preprocess the main image
